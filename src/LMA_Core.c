  ...
  if (p_phase->voltage.v_zc.v_sync_zc && !p_phase->disable_acc)
  {
    ...
    if (p_phase->voltage.v_zc.zero_cross_counter >= p_config->update_interval)
    {
      ...
      if (!p_phase->calibrating)
      {
        ...
        /* Active Power (P)*/
        p_phase->power.p = LMA_Fast_FPDiv(
            LMA_Fast_FPDiv(LMA_AccToFloat(p_phase->power.p_acc), p_phase->voltage.v_sample_counter_fp), p_phase->calib.p_coeff);
        /* Reactive Power (Q)*/
        p_phase->power.q = LMA_Fast_FPDiv(
            LMA_Fast_FPDiv(LMA_AccToFloat(p_phase->power.q_acc), p_phase->voltage.v_sample_counter_fp), p_phase->calib.p_coeff);

        /* Active Power (P) & Energy*/
        if (LMA_Fast_FPAbs(p_phase->power.p) < p_config->no_load_p)
        {
        ...
        {
          p_phase->status &= ~LMA_NO_ACTIVE_LOAD;
          p_phase->energy.act_unit = LMA_Fast_FPDiv(p_phase->power.p, p_config->gcalib.fs);
        }

        /* Reactive Power (Q) & Energy*/
        if (LMA_Fast_FPAbs(p_phase->power.q) < p_config->no_load_p)
        {
        ...
        {
          p_phase->status &= ~LMA_NO_REACTIVE_LOAD;
          p_phase->energy.react_unit = LMA_Fast_FPDiv(p_phase->power.q, p_config->gcalib.fs);
        }
...
void LMA_PhaseCalibrate(LMA_PhaseCalibArgs *const calib_args)
{
  ...
  calib_args->p_phase->calib.vrms_coeff =
      LMA_Fast_FPSqrt((float)calib_args->p_phase->voltage.v_acc / calib_args->p_phase->voltage.v_sample_counter_fp) /
      calib_args->vrms_tgt;
  calib_args->p_phase->calib.irms_coeff =
      LMA_Fast_FPSqrt((float)calib_args->p_phase->current.i_acc / calib_args->p_phase->voltage.v_sample_counter_fp) /
      calib_args->irms_tgt;
...
float LMA_VrmsGet(LMA_Phase *const p_phase)
{
  ...
  p_phase->voltage.v_rms =
      LMA_Fast_FPDiv(LMA_Fast_FPSqrt(LMA_Fast_FPDiv(LMA_AccToFloat(v_acc_tmp), sc_tmp)), p_phase->calib.vrms_coeff);
...
float LMA_IrmsGet(LMA_Phase *const p_phase)
{
  ...
  p_phase->current.i_rms =
      LMA_Fast_FPDiv(LMA_Fast_FPSqrt(LMA_Fast_FPDiv(LMA_AccToFloat(i_acc_tmp), sc_tmp)), p_phase->calib.irms_coeff);
...
float LMA_FLineGet(LMA_Phase *const p_phase)
{
  ...
  p_phase->voltage.fline = LMA_Fast_FPDiv(p_config->gcalib.fline_coeff, LMA_AccToFloat(fline_acc_tmp));
